var Parser = require('./Parser.js'),
	QuerySelector = require('./QuerySelector.js'),
	fs = require('fs'),
	path = require('path'),
...
Renderer.prototype.writeComponent = function(node) {
	...
	this.importDistributedNodes(node, clone);
...
Renderer.prototype.importDistributedNodes = function(from, to) {
	//distributed nodes
	var distributedNodes = from.children.map(function(child) {
	...
	contents.forEach(function(content) {
		var query = content.attrs.select || '*';

		i = 0;
		while (dist = distributedNodes[i]) {
			i++;
			if (!QuerySelector.test(dist, query)) return

			i--;
			content.appendChild(dist);
			distributedNodes.splice(i, 1);
		}
	});
};
